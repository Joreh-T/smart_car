...
// @LastEditTime : 2022-05-26 14:51:42
...
uint8 Branch_Compare_Count = 0;
int judge;
uint16 Branch_Integrate = 0; // 三叉编码器积分
uint8 Branc_Turn_Dire = 0;   // 三叉转向方向
...
void main() {
    ...
    while (1) {
        ...
        if (1 == Engine_Power) {
            ...
            // 直路行驶
            if (-30 < servo_error && servo_error < 30 && Car_1_Enter_Branch != (CH573_Rec_Command & 0x3F)) {
                motor_pid.target = 35;
                servo_pid.kp = 2.33;
            ...
            // 进入三叉减速
            else if (2 == Branch_State || distance < 50) {
                motor_pid.target = 15;
            ...
            else if (Car_1_Enter_Branch == (CH573_Rec_Command & 0x3F)) {
                motor_pid.target = 20;
        ...
        oled_int16(70, 1, Branc_Turn_Dire);
